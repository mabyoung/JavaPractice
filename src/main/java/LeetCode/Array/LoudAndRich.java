...
public class LoudAndRich {
    ...
    public int[] loudAndRich(int[][] richer, int[] quiet) {
        ...
        for (int[] rich : richer) {
            List<Integer> list = map.computeIfAbsent(rich[1], k -> new ArrayList<>());
            list.add(rich[0]);
        }
        ...
        for (int i = 0; i < result.length; i++) {
            ...
            indexSet.add(i);
            indexSet.addAll(gee(map, i));
            result[i] = getQuieter(quiet, indexSet);
        }
        return result;
    }

    public Set<Integer> gee(Map<Integer, List<Integer>> map, Integer index) {
        if (cache.containsKey(index)){
            return cache.get(index);
        }
        Set<Integer> result = new HashSet<>();
        if (index == null || map.get(index) == null) {
            return result;
        }
        List<Integer> list = map.get(index);
        result.addAll(list);
        for (Integer item : list) {
            result.addAll(gee(map, item));
        }
        cache.put(index, result);
        return result;
    }

    public int getQuieter(int[] quiet, Set<Integer> indexSet) {
        int minValue = Integer.MAX_VALUE;
        int minIndex = -1;
        for (Integer index : indexSet) {
            if (quiet[index] < minValue) {
                minValue = quiet[index];
                minIndex = index;
            }
        }
        return minIndex;
